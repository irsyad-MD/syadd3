    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          m.plugin = name
          if (name === 'menu.js' && (m.sender === '6285883225704@s.whatsapp.net' || m.sender === '6285883225704@s.whatsapp.net')) m.reply('Hai syad!')
          ...
          //if(!user.owner || !user.rowner) return conn.reply(m.chat, `Maaf @${m.sender.split`@`[0]} \n*『 Info 』*\nKami Staff syadbot ingin melakukan backup data, dan sedikit meningkatkan performa bot.\nJadi, Kami dari Staff syadbot memutuskan untuk melakukan *maintenance*, jika ada yang mau ditanyakan, harap hubungi Real Owner, Terima Kasih\n\n_*Hormat Kami, Staff syadbot*_`, m, { mentions: [m.sender] })
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.sendButton(m.chat, `Limit anda habis, silahkan chat Owner atau beli melalui *${usedPrefix}buy*`, '©syadbot', { 'button[0]': 'Owner', 'row[0]': '.owner', 'button[1]': 'Buy', 'row[1]': '.buy', 'button[2]': 'Buy All', 'row[2]': '.buyall' }, m)
